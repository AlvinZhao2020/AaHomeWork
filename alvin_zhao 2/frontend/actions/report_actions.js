...
const receiveAllReports =reports=>({
...
export const requestReports= ()=>dispatach =>(
    ReportAPIUtil.fetchReports()
    .then(reports=>dispatach(receiveAllReports(reports)))
)

export const requestReport = (reportId) =>dispatach =>(
    ReportAPIUtil.fetchReport(reportId)
    .then(report=>dispatach(receiveReport(report)))
)

export const createReport = (report)=>dispatach =>(
    ReportAPIUtil.createReport(report)
    .then(report => dispatach(receiveReport(report)))
)

export const updateReport = (report) => dispatach => (
    ReportAPIUtil.updateReport(report)
        .then(report => dispatach(receiveReport(report)))
)

export const deleteReport = (reportId)=> dispatach=> (
    ReportAPIUtil.deleteReport(reportId)
    .then(()=>dispatach(removeReport(reportId)))
)